  ...
  static async create({
    ...
    try {
      ...
      let sqlCreateUser = `INSERT INTO users (Users_name, Users_password, User_status_FK_ID, Role_FK_ID, Users_createdAt, Users_updatedAt) 
                          VALUES (?, ?, ?, ?, NOW(), NOW())`;
      const [userResult] = await connect.query(sqlCreateUser, [
        username,
        hashedPassword,
        user_status_id,
        role_id,
      ...
      let sqlCreateOwner = `INSERT INTO owner (User_FK_ID, Owner_is_tenant, Owner_birth_date, Owner_createdAt, Owner_updatedAt) 
                           VALUES (?, ?, ?, NOW(), NOW())`;
      const [ownerResult] = await connect.query(sqlCreateOwner, [
        userId,
        is_tenant,
        birth_date,
      ]);
  ...
  static async update(
    ...
    try {
      ...
      if (userUpdateFields.length > 0) {
        ...
        let sqlUpdateUser = `UPDATE users SET ${userUpdateFields.join(
          ", "
        )} WHERE Users_id = ?`;
      ...
      if (ownerUpdateFields.length > 0) {
        ...
        let sqlUpdateOwner = `UPDATE owner SET ${ownerUpdateFields.join(
          ", "
        )} WHERE Owner_id = ?`;
      ...
      if (
        ...
        if (profileResult.length > 0) {
          ...
          if (profileUpdateFields.length > 0) {
            let sqlUpdateProfile = `UPDATE profile SET ${profileUpdateFields.join(
              ", "
            )} WHERE Profile_id = ?`;
  ...
  static async getOwnersWithDetails(includeInactive = false) {
    try {
      ...
      SELECT 
        ...
        -- ✅ Nombre REAL del propietario (del perfil)
