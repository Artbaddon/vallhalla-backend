...
class AnswerController {
  async register(req, res) {
    try {
      ...
      const answerId = await AnswerModel.create({ survey_id, question_id, user_id, value });
  ...
  async findBySurvey(req, res) {
    try {
      ...
      if (!survey_id) {
        return res.status(400).json({ success: false, error: "survey_id requerido" });
      }
  ...
  async delete(req, res) {
    try {
      ...
      res.status(200).json({ success: true, message: "Respuesta eliminada exitosamente" });
    } catch (error) {
